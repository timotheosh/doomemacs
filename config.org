...
emacsd_start()
{
    su ${emacsd_user:=nobody} -c "env HOME=/home/${emacsd_user} /usr/local/bin/emacs ${emacsd_args} --daemon=${emacsd_socket}"
}
emacsd_stop()
{
    su ${emacsd_user:nobody} -c "/usr/local/bin/emacsclient --socket ${emacsd_socket} --eval \"(kill-emacs)\""
}
run_rc_command "$1"
#+end_src
* Pending Packages
