...
#+begin_src emacs-lisp :tangle no
(if window-system
    (cond ((font-info (font-spec :family "Source Sans Pro" :size 18.0))      '(font-spec :family "Source Sans Pro" :size 18.0))
  ...
  '(font-spec :family "Source Sans Pro" :size 18.0))
#+end_src
#+begin_src emacs-lisp :noweb yes
(cond ((string= system-type "berkeley-unix") (setq doom-variable-pitch-font (font-spec :family "Source Sans 3" :size 18.0)))
      (t                                     (setq doom-variable-pitch-font <<variable-font()>>)))
#+end_src
** Mixed-pitch Fonts
*** Package
#+BEGIN_SRC emacs-lisp :tangle packages.el
...
(after! counsel
  ...
  (map! "C-c b"
        #'(lambda (arg)
            (counsel-projectile-switch-to-buffer arg)))

  (map! "C-c B" #'counsel-switch-buffer)
...
*** Config
#+begin_src emacs-lisp
(when (file-directory-p "~/programs/share/emacs/emacs-application-framework")
  (use-package! eaf
    :load-path "~/programs/share/emacs/emacs-application-framework"
    :custom
...
#+begin_src emacs-lisp :tangle no
;; This will all be nil if roswell is not iunstalled.
...
(when ros-path
  ...
  (defun ros-config (impl)
    (let ((retval (list  ros-path "-L" impl "-Q" "run")))
      (if (or (is-sbcl? impl)
              (is-ecl? impl))
          (list (intern impl) retval :coding-system 'utf-8-unix)
        (list (intern impl) retval))))
...
priority. The first value will be the default implementation.")
  ...
  (defvar *setup-lisp*
    (let ((output (shell-command-to-string "ros list installed")))
      (let ((result (mapcar (lambda (text) (first (split-string text "/" t)))
...
emacsd_start()
{
    su ${emacsd_user:=nobody} -c "env HOME=/home/${emacsd_user:=nobody} /usr/local/bin/emacs --daemon=${emacsd_socket:=server}"
}
emacsd_stop()
{
