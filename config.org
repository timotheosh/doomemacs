...
If we are inside a project, ~C-x C-f~ should use ~counsel-projectile-find-file~, else use ~counsel-find-file~.
#+BEGIN_SRC emacs-lisp
(defun my/find-file ()
  (interactive)
  (let ((project-root (projectile-project-root)))
    (if (and project-root
             (not (equal (split-string project-root "/" t) (split-string (getenv "HOME") "/" t))))
