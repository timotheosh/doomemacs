...
#+BEGIN_SRC emacs-lisp
;; Default for doom (setq doom-font (font-spec :family "monospace" :size 18))
...
(defun run-program ()
  (interactive)
  (let ((input (split-string (read-string "run command: "))))
    (make-process :name "run-program" :buffer nil :command input)))
